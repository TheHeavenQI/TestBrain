...
namespace BaseFramework {
    ...
    public class CoroutineTask : Task<CoroutineTask> {
        ...
        public CoroutineTask Delay(params IEnumerator[] enumerators) {
            streams.Add(PoolHelper.Pop<CoroutineTaskStream>().Set(CreateEnumerator(enumerators)));
            return this;
        }

        public CoroutineTask Delay(params YieldInstruction[] yieldInstructions) {
            streams.Add(PoolHelper.Pop<CoroutineTaskStream>().Set(CreateEnumerator(yieldInstructions)));
            return this;
        }

        public CoroutineTask Delay(params float[] delays) {
            streams.Add(PoolHelper.Pop<CoroutineTaskStream>().Set(CreateEnumerator(delays)));
            return this;
        }

        public CoroutineTask Delay(Func<bool> waitUntil) {
            streams.Add(PoolHelper.Pop<CoroutineTaskStream>().Set(new WaitUntil(waitUntil)));
            return this;
        }

        private IEnumerator CreateEnumerator(params float[] delays) {
        ...
        private IEnumerator CreateEnumerator(params IEnumerator[] enumerators) {
        ...
        private IEnumerator CreateEnumerator(params YieldInstruction[] yieldInstructions) {
        ...
        public override CoroutineTask Do(params Action[] actions) {
            foreach (Action action in actions) {
                streams.Add(PoolHelper.Pop<CoroutineTaskStream>().Set(action));
            }
            return this;
        }
        ...
        internal override CoroutineTask ExecuteInternal() {
            ...
            if (behaviour != null && behaviour.isActiveAndEnabled) {
                coroutine = behaviour.StartCoroutine(GetFinalEnumerator());
            }

            return this;
        }

        private IEnumerator GetFinalEnumerator() {
