...
namespace BaseFramework
{
    public static class ArrayExtension
    {
        ...
        public static T[] SubArray<T>(this T[] self, int length)
        {
            return self.SubArray(0, length);
        }

        public static T[] SubArray<T>(this T[] self, int sourceIndex, int length)
        {
            if (self.IsNullOrEmpty()
                || sourceIndex < 0 || sourceIndex >= self.Length
                || length < 0 || sourceIndex + length >= self.Length)
            {
                Log.W(typeof(ArrayExtension), "SubArray Error");
                return self;
            }
            T[] des = new T[length];
            Array.Copy(self, sourceIndex, des, 0, length);
            return des;
        }
    }
}
